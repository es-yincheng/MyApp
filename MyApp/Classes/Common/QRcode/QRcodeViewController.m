...
@interface QRcodeViewController ()<AVCaptureMetadataOutputObjectsDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, UIAlertViewDelegate>
...
- (void)setupTipTitleView{
    ...
    UIView*mask=[[UIView alloc]initWithFrame:CGRectMake(0, _maskView.frame.origin.y+_maskView.frame.size.height, self.view.frame.size.width, kBorderW)];
...
-(void)setupNavView{
    ...
    albumBtn.center=CGPointMake(self.view.frame.size.width/2, 20+49/2.0);
    ...
    flashBtn.frame = CGRectMake(self.view.frame.size.width-55,20, 35, 49);
...
- (void)setupMaskView
{
    ...
    mask.bounds = CGRectMake(0, 0, self.view.frame.size.width + kBorderW + kMargin , self.view.frame.size.width + kBorderW + kMargin);
...
{
    //1.下边栏
    UIView *bottomBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height * 0.9, self.view.frame.size.width, self.view.frame.size.height * 0.1)];
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    if (metadataObjects.count>0) {
        ...
        QRStringShowController *vc = [[QRStringShowController alloc] init];
        vc.QRString = metadataObject.stringValue;
...
- (void)myCode{
    ...
    NSLog(@"我的二维码");
    MyQRController *vc=[[MyQRController alloc]init];
    [self.navigationController pushViewController:vc animated:YES];
